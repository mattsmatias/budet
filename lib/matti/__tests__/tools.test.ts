...
describe("syötteen validointi", () => {
  ...
  it("hylkää kelvottoman päivän", () => {
    ...
    expect(
      tool.schema.safeParse({ weekStart: "24.8.2026", euros: 16 }).success,
    ).toBe(false);
    expect(
      tool.schema.safeParse({ weekStart: "2026-08-24", euros: 16 }).success,
    ).toBe(true);
  });
  ...
  it("hylkää negatiivisen hinnan", () => {
    const tool = findTool("propose_lunch_price")!;
    expect(
      tool.schema.safeParse({ weekStart: "2026-08-24", euros: -5 }).success,
    ).toBe(false);
  });
