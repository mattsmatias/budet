...
export async function generateMetadata({
  params,
}: PageProps<"/kehittaja/yritykset/[id]">) {
  const { id } = await params;
  const detail = await fetchRestaurant(id);
  return { title: detail?.restaurant.name ?? "Yritys" };
}
    ...
    <div className="rf-stagger space-y-5">
      <header>
        ...
        <div className="mt-1 flex flex-wrap items-center gap-3">
          ...
          {r.isTestAccount ? <Pill tone="info">Testiyritys</Pill> : null}
        </div>
        ...
        {VALILEHDET.map((v) => {
          ...
          return (
            <Link
              ...
              href={
                v.key === "yleiskatsaus"
                  ? `/kehittaja/yritykset/${id}`
                  : `/kehittaja/yritykset/${id}?valilehti=${v.key}`
              }
        ...
        <div className="space-y-4">
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <Card>
              <CardHeader
                title="Omistaja"
                subtitle="Kuka vastaa tästä yrityksestä"
              />
              ...
              {owner ? (
                ...
                <p
                  ...
                  style={{ color: "var(--rf-amber-text)" }}
                >
                  Yrityksellä ei ole aktiivista omistajaa. Luo kutsu
      ...
      {valittu === "kaytto" ? (
        <div className="space-y-4">
          <section className="grid auto-rows-fr rf-stat-grid grid-cols-2 gap-2.5 sm:gap-3.5 xl:grid-cols-3">
            <MetricCard
              label="Kuitteja"
              value={usage.receipts}
              icon={<RfIcon name="receipt" size={17} />}
